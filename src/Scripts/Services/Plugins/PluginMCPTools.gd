...
func _get_plugin_build_status_tool_def() -> Dictionary:
	return {
		...
		"description": "Get a plugin's manifest-install setup-pipeline state. Returns state/state_name (BUILDING, BUILD_FAILED, NEEDS_BINARY, or a normal lifecycle state), live progress {step_index, step_count, step_type} while building, the full failure envelope on BUILD_FAILED/NEEDS_BINARY (including the per-tool `failures` array and `detail: exec_denied` when the user declined an exec step), and the step-by-step log of the most recent build. Also returns install_lane and rebuildable. Rebuildable is true only for a manifest/dev plugin with a setup recipe; marketplace and host-owned plugins require their documented reinstall or source repair path. Poll this after minerva_plugin_install returns {building: true}.",
		"input_schema": {
			"type": "object",
			"properties": {
				"id": {
