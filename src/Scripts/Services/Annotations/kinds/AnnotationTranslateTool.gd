...
## rotate + scale, no mode switching); its Zone.INSIDE drag is this behavior.
## AnnotationToolbar._construct_tool_for_name no longer builds this class and
## nothing else constructs it outside tests. Kept on disk only so its tests and
## history stay readable; slated for deletion. Do NOT re-list it in a toolbar
## and do NOT add features here.
##
