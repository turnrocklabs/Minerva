...
## [br][br]
## You can still use BBCode tags that don't have a Markdown equivalent, such as `[u]underlined text[/u]`, allowing you to have the full functionality of RichTextLabel with the simplicity and readability of Markdown.
## [br][br]
## Check out the full guide in the Github repo readme file (linked below). If encountering any unreported bug or unexpected behavior, please ensure that your Markdown is written as clean as possible, following best practices.
## 
...
## Initializes the MarkdownLabel with optional text
## @param markdown_text The initial markdown text
func _init(new_markdown_text: String = "") -> void:
	bbcode_enabled = true
	self.markdown_text = new_markdown_text
	if automatic_links:
...
## Handles automatic link clicks
## @param meta The metadata associated with the clicked link
...
## Validates specific properties to hide them in the editor.
...
func display_text(_display_text: String) -> void:
	markdown_text = _display_text

#endregion

#region Private methods:

## Updates the label with the converted markdown text.
func _update() -> void:
...
## Sets the markdown text and triggers an update.
## @param new_text The new markdown text
...
## Sets the format for level-1 headers and triggers an update.
## @param new_format The new format for level-1 headers
func _set_h1_format(new_format: H1Format):
	h1 = new_format
	_update()

## Sets the format for level-2 headers and triggers an update.
## @param new_format The new format for level-2 headers
func _set_h2_format(new_format: H2Format):
	h2 = new_format
	_update()

## Sets the format for level-3 headers and triggers an update.
## @param new_format The new format for level-3 headers
func _set_h3_format(new_format: H3Format):
	h3 = new_format
	_update()

## Sets the format for level-4 headers and triggers an update.
## @param new_format The new format for level-4 headers
func _set_h4_format(new_format: H4Format):
	h4 = new_format
	_update()

## Sets the format for level-5 headers and triggers an update.
## @param new_format The new format for level-5 headers
func _set_h5_format(new_format: H5Format):
	h5 = new_format
	_update()

## Sets the format for level-6 headers and triggers an update.
## @param new_format The new format for level-6 headers
func _set_h6_format(new_format: H6Format):
	h6 = new_format
	_update()

## Converts markdown text to BBCode text.
## @param source_text The markdown text to convert
...
func _convert_markdown(source_text = "") -> String:
	...
	for line in lines:
		...
		if within_code_block:
			# Ignore any formatting inside code block
...
## Processes list syntax in a line.
## @param line The line to process
## @param indent_spaces The list of indent spaces
## @param indent_types The list of indent types
...
## Escapes BBCode in the source text.
...
## Escapes specific characters in the text.
...
## Resets escaped characters in the text.
## @param _text The text to process
## @param code Whether to escape BBCode characters
## @return The text with escaped characters reset
...
## @param string The debug string to print
func _debug(string: String):
	if not _debug_mode:
		return
	print(string)

## Checks if a line denotes a fenced code block.
## @param line The line to check
## @param character The character used for the code block
## @return Whether the line denotes a fenced code block
func _denotes_fenced_code_block(line: String, character: String) -> bool:
	...
	if count >= 3 and count == stripped_line.length():
		...
		for i in count:
			block_tag += character
		if stripped_line.begins_with(block_tag):
			return true
		return false
	else:
		return false

## Gets the character count for a code block.
## @param line The line to check
## @param character The character used for the code block
## @return The character count
func _get_codeblock_char_count(line: String, character: String) -> int:
	var stripped_line := line.strip_edges()
	return stripped_line.count(character)

## Processes table syntax in a line.
## @param line The line to process
## @return The processed line
...
## Gets the format for a header level.
## @param level The header level
## @return The format for the header level
...
## Gets the tags for a header format.
## @param header_format The header format
## @param closing Whether to get closing tags
...
## Gets the reference for a header.
## @param header_string The header string
## @return The header reference
