...
func _draw_editing_tail() -> void:
	var closest_point: = ellipse[get_closest_ellipse_line(get_viewport().get_mouse_position())]
	if get_viewport().get_mouse_position().distance_to(closest_point) < 60:
		draw_circle(closest_point, 4, Color.DEEP_PINK)
	else:
		draw_circle(get_viewport().get_mouse_position(), 4, Color.DEEP_PINK)
