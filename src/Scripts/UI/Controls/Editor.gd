...
static func create(type_: Type, file_ = null, name_ = null, associated_object_ = null, initial_setup: = true) -> Editor:
	...
	match type_:
		...
		Editor.Type.WEBVIEW:
			# Web panels use the sandboxed CEF document host. The editor scene shows
			# a clear unavailable message when the native extension is missing.
			var webview_editor_scene = load("res://Scenes/CefWebViewEditor.tscn")
