...
func _input(event: InputEvent):
	...
	if event is InputEventMouseButton:
		...
		if is_drawing and event.pressure > 0 and _is_mouse_in_drawing_zone(mouse_position) and not mouse_over_color_palette and not mouse_over_buttons_panel:
			_DrawZone.visible = true
			_DrawZone.position = mouse_position
			smooth_line(last_position, last_pressure, mouse_position, event.pressure)
			last_pressure = event.pressure 
			Input.set_mouse_mode(Input.MOUSE_MODE_HIDDEN) 
		else:
			_DrawZone.visible = false
			Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE) 
...
func create_point(position: Vector2, pressure: float):
	...
	_DrawZone.size = Vector2(Bsize, Bsize)
...
func erase_segments_from_line(line: Line2D, point: Vector2):
	for i in range(line.get_point_count() - 1):
		...
		if start.distance_to(point) <= erase_radius or end.distance_to(point) <= erase_radius:
			line.queue_free()
			break # Important: Exit the loop after deleting a segment
...
func _is_mouse_in_drawing_zone(mouse_position: Vector2) -> bool:
	"""Checks if the mouse is within the bounds of the _ColorRect."""
