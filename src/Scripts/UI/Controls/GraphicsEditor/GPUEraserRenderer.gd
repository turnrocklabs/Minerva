class_name GPUEraserRenderer
extends RefCounted

## GPU-accelerated eraser rendering using compute shaders
## Optimizes erasing operations by offloading pixel manipulation to the GPU
...
func _init():
	...
	if not rd:
		push_error("Failed to create local RenderingDevice for GPU eraser rendering")
		return
...
func _load_shader() -> void:
	var shader_file = load("res://Scripts/UI/Controls/GraphicsEditor/eraser_compute.glsl")
	var shader_spirv: RDShaderSPIRV = shader_file.get_spirv()
	
	if not shader_spirv:
		push_error("Failed to load eraser compute shader SPIRV")
		return
...
func initialize_buffers(image: Image, selection_data: PackedByteArray = PackedByteArray()) -> bool:
	...
	# Create layer image buffer
	layer_image_rid = _create_image_buffer(image)
...
func _create_uniform_set() -> void:
	...
	# Binding 0: layer_image (image2D)
	var u0 = RDUniform.new()
	u0.uniform_type = RenderingDevice.UNIFORM_TYPE_IMAGE
	u0.binding = 0
	u0.add_id(layer_image_rid)
	uniforms.append(u0)
	
	# Binding 1: params (storage buffer)
	var u1 = RDUniform.new()
	u1.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER
	u1.binding = 1
	u1.add_id(params_buffer_rid)
	uniforms.append(u1)
	
	# Binding 2: selection (storage buffer)
	var u2 = RDUniform.new()
	u2.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER
	u2.binding = 2
	u2.add_id(selection_buffer_rid)
	uniforms.append(u2)
	
	uniform_set = rd.uniform_set_create(uniforms, shader, 0)


func erase_at(center: Vector2, radius: float, use_selection: bool = false) -> void:
	if not rd or not uniform_set.is_valid():
		return
	
	# Update parameters buffer
	_update_params_buffer(center, radius, use_selection)
	
	# Dispatch compute shader
	_dispatch_compute()


func _update_params_buffer(center: Vector2, radius: float, use_selection: bool) -> void:
	...
	# float _padding[2] (8 bytes) - already zeroed
...
func get_erased_image() -> Image:
	if not rd or not layer_image_rid.is_valid():
		return null
	
	var byte_data = rd.texture_get_data(layer_image_rid, 0)
...
func cleanup() -> void:
	...
	if layer_image_rid.is_valid():
		rd.free_rid(layer_image_rid)
