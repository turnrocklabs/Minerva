...
func _update_diagram_text_edit_style() -> void:
	if not _diagram_text_edit:
		return

	var font = ThemeDB.get_fallback_font()
	var font_size = int(text_size_spin_box.value) if text_size_spin_box else 14
...
func _on_text_size_changed(value: float) -> void:
	# Update inline edit style if editing
	if _editing_diagram_shape and _diagram_text_edit:
		_update_diagram_text_edit_style()
		_center_diagram_text_edit(_editing_diagram_shape)
		_diagram_text_edit.grab_focus()
