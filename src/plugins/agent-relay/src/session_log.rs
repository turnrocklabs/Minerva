...
// cwd, the window the terminal was alive, the prompt texts sent into it) and
// searches the harness's own log tree READ-ONLY. Nothing is remembered: the
// same inputs re-derive the same answer, so a relay restart loses no binding.
// When two files survive every filter it refuses (Ambiguous) rather than
// picking one — a wrong transcript is worse than none.
//
...
// Nothing calls the binder yet; drop this when it joins the turn path.
#![allow(dead_code)]
...
pub struct TerminalFacts {
    ...
    /// Exact prompt bodies the relay submitted into this terminal.
...
/// → alive-window overlap → codex originator → every sent prompt present.
/// Survivors are compared by path, so the outcome does not depend on directory
/// iteration order.
pub fn bind(facts: &TerminalFacts, roots: &LogRoots) -> Binding {
    let harness = match facts.profile_id.as_str() {
        "claude" => Harness::Claude,
        "codex" => Harness::Codex,
        other => return Binding::Unsupported(other.to_string()),
    };
    ...
    // Prompt evidence is the only discriminator between sessions that share a
    // cwd and overlap in time. Without it, siblings stay ambiguous.
    if facts.prompts.is_empty() {
        return resolve(surviving);
    }
    let matched: Vec<PathBuf> = surviving
        .into_iter()
        .filter(|path| holds_all_prompts(path, harness, &facts.prompts))
        .collect();
    resolve(matched)
}
...
fn scan_head(path: &Path, harness: Harness) -> Head {
    ...
    for record in read_records(path).take(HEAD_RECORDS) {
        match harness {
            Harness::Claude => {
                if head.cwd.is_none() {
                    head.cwd = record
...
fn holds_all_prompts(path: &Path, harness: Harness, prompts: &[String]) -> bool {
    ...
    prompts
        .iter()
        .all(|prompt| texts.iter().any(|text| prompt_matches(prompt, text)))
}
...
fn prompt_matches(prompt: &str, text: &str) -> bool {
...
/// Stream a .jsonl file as records, skipping lines that do not parse. Bytes
/// are decoded lossily so a corrupt line cannot hide the records around it.
fn read_records(path: &Path) -> impl Iterator<Item = Value> {
    let mut reader = fs::File::open(path).ok().map(BufReader::new);
