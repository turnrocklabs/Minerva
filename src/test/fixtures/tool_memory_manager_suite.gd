extends RefCounted
## Runtime-autoload test suite loaded by test_tool_memory_manager.gd.
...
class SummaryFixtureManager extends ToolMemoryManager:
	var summary_text := ""

	func _get_agent_note_text(note_id: String) -> String:
		return summary_text if not note_id.is_empty() else ""
...
func run() -> Dictionary:
	...
	test_floating_summary_reaches_retained_latest_tool()
	...
	return {"passed": _pass_count, "failed": _fail_count, "skipped": _skip_count}
...
func test_floating_summary_reaches_retained_latest_tool():
	if not require_chi("floating_summary_reaches_retained_latest_tool"): return
	var tmm := SummaryFixtureManager.new()
	tmm.enabled = true
	tmm.summary_text = "prior tool facts"
	var history := ServiceHistory.new(null, "projection-summary-test")
	history.HistoryItemList = _make_3_round_history()
	history.AgentFloatingSummaryNoteId = "summary-note"
	var source_latest: ChatHistoryItem = history.HistoryItemList.back()

	var projected := tmm.project(history, null)
	var projected_latest: ChatHistoryItem = projected.back()

	check("retained latest tool receives floating summary and current result",
		projected_latest.Message == "[tm]\nprior tool facts\n\n[cur]\nAdded row for Anker cable")
	check("retained tool protocol identity survives summary injection",
		projected_latest.Role == ChatHistoryItem.ChatRole.TOOL
		and projected_latest.ToolCallId == "call_3"
		and projected_latest.ToolName == "add_to_spreadsheet")
	check("projection does not mutate persisted latest tool",
		projected_latest != source_latest and source_latest.Message == "Added row for Anker cable")
