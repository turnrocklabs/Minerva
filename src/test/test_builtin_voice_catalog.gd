...
func _init() -> void:
	...
	check("trusted voice registration succeeds", db.register_internal() and db.get_by_id("voice").data_directory == trusted.data_directory)
	check("caller definition cannot replace trusted voice path", db.register_internal(hostile) and db.get_by_id("voice").entrypoint == trusted.entrypoint and db.get_by_id("voice").data_directory == trusted.data_directory)
