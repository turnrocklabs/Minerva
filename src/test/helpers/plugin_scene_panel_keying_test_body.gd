...
func test_editor_annotation_reparent_preserves_ipc_registration() -> void:
	...
	get_tree().root.add_child(detached_panel)
	...
	await get_tree().process_frame
	await get_tree().process_frame
	get_tree().root.remove_child(detached_panel)
	await get_tree().process_frame
...
func test_reply_after_reregistration_is_dropped() -> void:
	...
	get_tree().root.add_child(panel_new)
	...
	await get_tree().process_frame
	get_tree().root.remove_child(panel_new)
	panel_new.free()
	panel_old.free()
...
class StubAnnotationSceneRoot extends StubSceneRoot:
	var annotation_host := StubAnnotationHost.new()

	func get_annotation_host() -> AnnotationHost:
		return annotation_host


## Editor mounts the complete dock and overlay, so the fixture host must honor
## the same registry and change-signal surface as a production annotation host.
class StubAnnotationHost extends AnnotationHost:
