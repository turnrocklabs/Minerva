...
var temp_type_holder: Type # this needs to be moved probably to the editor
# Request redraw in response to changes
func _ready():
	circle_radius = 50
	if temp_type_holder == Type.SPEECH_BUBBLE:
		type = Type.SPEECH_BUBBLE
	elif temp_type_holder == Type.ELLIPSE:
		type = Type.ELLIPSE
	elif temp_type_holder == Type.RECTANGLE:
		type = Type.RECTANGLE
