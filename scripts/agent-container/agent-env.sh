...
# Toolchains are the image's (read-only /opt, /usr/local/go); cargo's and Go's
# module caches persist per session. The tools dir is agent-upgrade's.
export CARGO_HOME=/agent-home/cargo GOPATH=/agent-home/go
export PATH="${MINERVA_AGENT_TOOLS:-/agent-home/tools}/bin:$PATH:${MINERVA_AGENT_DIR:-/opt/minerva-agent}"
