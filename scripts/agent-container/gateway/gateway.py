...
For every session in the sessions file it creates, inside that session's own
socket directory:
...
on TCP; the upstreams are the host's loopback MCP services.
...
SESSION_KEYS = {"terminal_id", "harness", "socket_dir", "notify_targets", "docket_projects"}
...
def parse_session(entry):
    ...
    if not isinstance(entry["terminal_id"], str) or not TERMINAL_ID.fullmatch(entry["terminal_id"]):
        raise ValueError("terminal_id malformed")
    if entry["harness"] not in ("claude", "codex"):
        raise ValueError("harness must be claude or codex")
    for key in ("notify_targets", "docket_projects"):
    ...
    session = mcp_policy.Session(entry["terminal_id"], entry["harness"],
                                 frozenset(entry["notify_targets"]),
                                 frozenset(entry["docket_projects"]))
...
def build_servers(config, sessions_data, resolve=connect_proxy.system_resolve,
    ...
    ids = [s.terminal_id for s, _ in sessions]
    ...
    for session, sock_dir in sessions:
        ...
        servers.append(connect_proxy.make_server(str(sock_dir / "proxy.sock"),
                                                 session.terminal_id, allowlist, resolve, connect))
    return servers
