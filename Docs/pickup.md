...
| 5 | 2026-05-27 | (this commit) | **Layer B v4 — instrumentation push.** Wrapped Godot launch in `stdbuf -oL -eL` to force line-buffered stdout/stderr (locally Minerva produces megabytes of output; CI showed <4KB — pure buffering, not actual silence). Added `--verbose` for richer startup detail. Added live `tail -30 minerva.log` dumps every 15s while polling for :9315, so a hung startup leaves a trail in the GH UI rather than one terminal failure. Saved durable nudge `minerva-ci/godot-stdout-buffering`. CI result pending — even if it still fails, we'll have much better diagnostics next iteration. |
