...
| 3 | 2026-05-27 | (this commit) | **Layer B v2 — local end-to-end PROVEN.** Swapped xvfb-run → `--headless` (the existing `Smoke-launch Linux build` step already uses --headless successfully). Confirmed locally: headless Minerva opens :9315, exposes `minerva_plugin_marketplace_install`, and ALL three steps (install + start + list state=RUNNING) work end-to-end via curl. Also discovered: Minerva's MCP HTTP server is **session-less** — no `mcp-session-id` returned in initialize response (iter-2 smoke would have failed at session capture even with HTTP up); saved nudge hint `minerva-mcp-http/no-session-id-header`. Smoke script v2 drops session-id requirement, dumps FULL minerva.log on failure, uploads it as `tarball-smoke-minerva-log` artifact. Apt deps simplified (dropped xvfb + libasound2t64; kept libnss3+libgbm1 for CEF). CI result pending. |
