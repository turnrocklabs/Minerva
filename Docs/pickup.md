...
| 9 | 2026-05-27 | (this commit) | **REAL ROOT CAUSE.** `SingletonObject._ready` at line 1240 had `var err = config_file.load(_config_file_name); if err != OK: return null`. Fresh CI user-data dir → no `user://config_file.cfg` → ERR_FILE_NOT_FOUND → `_ready` early-returns BEFORE `initialize_mcp.call_deferred()`, `_init_docket()`, `_init_agent_system()`. Locally hidden because my workstation has an accumulated config from many runs. **Latent bug for every fresh install.** Fix: on missing config, push_warning + save empty config + continue. Removed iter-7/8 [MCP-DEBUG] instrumentation now that root cause is known. Iter-6 idempotency fix kept (defensive against the lazy-create race that does still occur). Layer A still 7/0. Saved durable nudge `minerva-singleton/ready-bails-on-missing-config-file`. |
