...
| 6 | 2026-05-27 | (this commit) | **Minerva idempotency fix.** Root cause: `SingletonObject.initialize_mcp()` did `if mcp_manager: return` — anyone calling `get_mcp_manager()` (lazy init) before the deferred initialize_mcp fired silently skipped `connect_minerva_server()` AND `start_http_server()`. Result in CI: no MCP HTTP on :9315, no Minerva MCP server registered, no error. Locally timing was favorable; CI ubuntu-latest hit it reliably. Fix: split the early-return so only the `mcp_manager = .new() + add_child` is conditional; connect_minerva_server + start_http_server now always run (both already idempotent). Layer A still 7/0 with the fix. Saved durable nudge `minerva-singleton/initialize-mcp-was-not-idempotent`. CI result pending. User extended budget by +6 iterations after iter-5. |
