...
| 4 | 2026-05-27 | (this commit) | **Layer B v3 — xvfb + --headless together (local proof).** Smoke now launches under `xvfb-run --auto-servernum --server-args='-screen 0 1024x768x24' Minerva.x86_64 --headless`. xvfb supplies the X socket gdcef_helper children need for ANGLE/EGL handshake; --headless prevents Godot from creating its own window. End-to-end probed locally against this exact invocation: init handshake ok → marketplace install ok → start ok → scansort RUNNING in <20s. Apt deps: re-added xvfb. CI result pending. |
