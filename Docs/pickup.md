...
LATEST **2026-07-19** — **hermetic-CAM DCR `019f761ead82`; Stage 2 pad-bug PROVEN FIXED via the live oracle. Steps 1–4a-i DONE + owner re-bless DONE.** Stage 2 migration path D: **S1 transform→geometry.py `369937e` · S2 A1 FootprintDefinition+fixtures `cd5d2e7` · S3 chain resolve into fab behind default-OFF gate `1577d1d` · S4a-i spike resolves to real 0805 land + golden regen `baae81a` · owner re-bless → correctness oracle LIVE `0e10b87`**. The strict-xfail for pad bug `019f7736b236` is GONE — now a LIVE PASS: emitter(resolved spike) == owner-re-blessed golden EXACTLY on copper/mask/drill/edge. Suite **409 pass / 1 skip / 0 xfail**. Pad bug CLOSED in the ORACLE sense (kept OPEN pending 4a-ii product closure). NEXT = **Step 4a-ii** (headless, base `0e10b87`) — **owner-ratified design (2): best-effort resolve + EMITTER fail-closed.** Plan: (a) remove placeholder defaults (gerber `DEFAULT_SMD_PAD_W/H_MM`, kicad `1`/`0.6`, route_bridge `_DEFAULT_PAD_SIZE`) → fail-closed lives in `pad_source.iter_pads` (pad with NO geometry from resolved OR inline → raise); (b) methods gate default ON with a TOLERANT fab-path resolve (enrich resolvable components, leave others to inline pins, don't fail the whole board on an unresolvable footprint; standalone `_resolve` action stays STRICT); (c) `test_gerbers` builds the spike board RESOLVED → regenerate `gerber_golden/board-*` (DRIFT pins, not blessed → ruling-4 diff-empty vs the blessed spike golden), drilltest golden UNCHANGED (all-TH inline, no SMD). Then `019f7736b236` FULLY closes. **Fallout is small**: drilltest has NO SMD pads (all TH inline) so it's unaffected by placeholder removal — the only affected board is the spike (SMD, no inline). Then **Step 4b** = canvas-degrade `.gd` VISUAL HITL. Design fork + ratification: docket `019f761fe518` comments.
