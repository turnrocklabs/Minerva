...
| 7 | Propose button discoverability + status wording ("N proposals" / "nothing to route") in a real session | E2E scenario B/D assert exact status strings + real-worker proposal creation (test_pcb_explicit_propose.gd 64/0) | C5 |
| 8 | Accept/Reject inline row buttons in the workflow dock: placement, sizing, and whether "Accept deletes the scaffolding" is understood without a tooltip | Scenario B2 asserts buttons appear only on AI-authored rows and wire to the real host verbs; non-pcb dock renders unchanged (annotation_dock suite 14/0) | C5 |
| 9 | Backend-stopped message wording matches what a human actually clicks ("Start it from the Plugin Manager") | Scenario E asserts the exact string + structured pcb_backend_stopped error with recovery_hint | C5 |
