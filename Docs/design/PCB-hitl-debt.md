...
| 9 | Route-correction loop live pass (AFTER go-live wiring 019f3815e9f9): author route hints in a `.pcbskel`, run `minerva_pcb_apply_route_hints` → cyan AI proposal polylines appear in the dock linked to the source hints, board unchanged; `commit:true` lays real traces + flips source hints to `applied`; add hints for a stuck net → re-run picks up only the new ones | apply flow 47/0 with the worker route STUBBED (canned RoutingResult); real worker hop unwired until 019f3815e9f9 | Apply/write-back round (21dcf094 / 0e73cc6) |
