# Third-Party Licenses

Minerva uses the following third-party libraries. All use permissive licenses.

## godot-cpp
- **License:** MIT
- **Copyright:** 2017-present Godot Engine contributors
- **Source:** https://github.com/godotengine/godot-cpp

## Ghostty (libghostty-vt)
- **License:** MIT
- **Copyright:** 2024 Mitchell Hashimoto, Ghostty contributors
- **Source:** https://github.com/ghostty-org/ghostty

## EIRTeam.FFmpeg
- **License:** MIT
- **Copyright:** 2018 Alex Roman (EIRTeam)
- **Source:** https://github.com/EIRTeam/EIRTeam.FFmpeg

## FFmpeg
- **License:** LGPL 2.1 (used unmodified, dynamically linked)
- **Copyright:** FFmpeg developers
- **Source:** https://ffmpeg.org
- **Note:** Minerva links against FFmpeg shared libraries without modification.
  LGPL 2.1 permits this without requiring Minerva itself to be open-sourced.

## Zig (build tool)
- **License:** MIT
- **Copyright:** Zig Software Foundation
- **Source:** https://ziglang.org

## SCons (build tool)
- **License:** MIT
- **Copyright:** SCons Foundation
- **Source:** https://scons.org

## Bun (optional, Stream Deck plugin compiler)
- **License:** MIT
- **Copyright:** Oven (Jarred Sumner)
- **Source:** https://bun.sh

## jsoncons
- **License:** Boost Software License 1.0
- **Copyright:** Daniel Parker and contributors
- **Source:** https://github.com/danielaparker/jsoncons (v1.9.0; exact commit and archive hash in `src/native/json_schema_helper/dependency-lock.json`)

## Model Context Protocol schema
- **License:** MIT
- **Copyright:** Anthropic, PBC
- **Source:** https://github.com/modelcontextprotocol/modelcontextprotocol (exact commit and artifact hashes in `src/native/json_schema_helper/dependency-lock.json`)
